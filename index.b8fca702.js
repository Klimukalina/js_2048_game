...
const game = new Game();
const { startButton, gameScore, gameBoardRows, messageLose, messageWin, messageStart } = {
    ...
    moveLeft() {
        if (this.status === Game.STATUS.PLAYING) {
            ...
            for(let row = 0; row < size; row++){
                ...
                for(let col = 0; col < size; col++)if (this.state[row][col] !== 0) newRow.push(this.state[row][col]);
                ...
                for(let col = 0; col < size; col++)if (this.state[row][col] !== newRow[col]) {
    ...
    moveUp() {
        if (this.getStatus() === Game.STATUS.PLAYING) {
            ...
            for(let col = 0; col < size; col++){
                const newColumn = [];
                for(let row = 0; row < size; row++)if (this.state[row][col] !== 0) newColumn.push(this.state[row][col]);
                ...
                for(let row = 0; row < size; row++)if (this.state[row][col] !== newColumn[row]) {
